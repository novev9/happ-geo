# happ-geo

Готовый профиль маршрутизации для [Happ](https://www.happ.su): весь трафик через VPN, кроме российских сервисов и Apple/Microsoft/Steam. DNS: Cloudflare `1.1.1.1` over DoH.

## Установка

1. Установить Happ из App Store.
2. Тапнуть ссылку на iPhone (или скопировать и импортировать в Happ → Routing → Import from clipboard):

```
happ:
```

3. Подключить VPN (нужна своя подписка — этот репо её не содержит).

## Источники

Geo-файлы — зеркало [hydraponique/roscomvpn-geosite](https://github.com/hydraponique/roscomvpn-geosite) (MIT) и [roscomvpn-geoip](https://github.com/hydraponique/roscomvpn-geoip), запинены по коммиту для стабильности.

Лицензия: MIT ([LICENSE](./LICENSE)).
